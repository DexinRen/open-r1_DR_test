...
class EarlyStoppingOnMetricDropCallback(TrainerCallback):
    def __init__(self, monitor="eval_loss", mode="min", patience=2):
    ...
    def on_evaluate(self, args, state, control, metrics=None, **kwargs):
        ...
        if improved:
            ...
            self.counter += 1
            print(f"[EarlyStopping] No improvement in {self.monitor}. Counter = {self.counter}/{self.patience}")

        if self.counter >= self.patience:
            control.should_training_stop = True
            print(f"[EarlyStopping] Stopping training early at step {state.global_step} due to {self.monitor} not improving.")
